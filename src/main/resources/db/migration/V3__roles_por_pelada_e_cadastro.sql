...
SELECT nextval('pelada_membros_id_seq'), p.id, p.organizador_id, 'ADMIN', TRUE, now()
FROM peladas p;

INSERT INTO pelada_membros (id, pelada_id, usuario_id, papel, ativo, criado_em)
SELECT nextval('pelada_membros_id_seq'), pj.pelada_id, j.usuario_id, 'JOGADOR', TRUE, now()
...
SELECT setval(pg_get_serial_sequence('pelada_membros', 'id'), COALESCE(MAX(id), 0) + 1, false)
FROM pelada_membros;
